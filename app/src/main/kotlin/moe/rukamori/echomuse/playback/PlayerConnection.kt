...
import moe.rukamori.echomuse.extensions.getCurrentQueueIndex
...
import moe.rukamori.echomuse.ui.player.refetchCanvasArtworkForPlayback
...
class PlayerConnection(
    ...
    fun seekToNext() {
        val state = service.togetherSessionState.value as? moe.rukamori.echomuse.together.TogetherSessionState.Joined
        if (state?.role is moe.rukamori.echomuse.together.TogetherRole.Guest) {
            service.requestTogetherControl(moe.rukamori.echomuse.together.ControlAction.SkipNext)
            return
        }
        player.seekToNext()
        player.prepare()
    ...
    fun seekToPrevious() {
        val state = service.togetherSessionState.value as? moe.rukamori.echomuse.together.TogetherSessionState.Joined
        if (state?.role is moe.rukamori.echomuse.together.TogetherRole.Guest) {
            service.requestTogetherControl(moe.rukamori.echomuse.together.ControlAction.SkipPrevious)
            return
        }
